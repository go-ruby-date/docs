...
# `go-ruby-date` library-level benchmark harness

Reproducible, cross-runtime benchmark of the **pure-Go `go-ruby-date` library**
...
reference runtime's own `date`?*

## Layout

- `go/`          — self-contained Go driver; `go.mod` pins the published library.
- `ruby/date.rb`  — the equivalent workload; `ruby/_harness.rb` is the shared timer.
