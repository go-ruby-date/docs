...
pure-Go implementation as fast as the reference runtime's own `date`?* The
...
| --- | ---: | ---: |
| **go-ruby (pure Go)** | 766.9 | 0.43× |
| MRI | 1775.5 | 1.00× |
| MRI + YJIT | 1749.5 | 0.99× |
| JRuby | 779.1 | 0.44× |
| TruffleRuby | 30673.2 | 17.28× |

#### plus-days

| Runtime | ns/op | vs MRI |
| --- | ---: | ---: |
| **go-ruby (pure Go)** | 8.3 | 0.15× |
| MRI | 54.8 | 1.00× |
| MRI + YJIT | 29.2 | 0.53× |
| JRuby | 38.6 | 0.70× |
| TruffleRuby | 1925.5 | 35.14× |

#### strftime

| Runtime | ns/op | vs MRI |
| --- | ---: | ---: |
| **go-ruby (pure Go)** | 213.8 | 1.05× |
| MRI | 203.5 | 1.00× |
| MRI + YJIT | 168.5 | 0.83× |
| JRuby | 340.9 | 1.68× |
| TruffleRuby | 1855.8 | 9.12× |

`parse` (0.43×) and day arithmetic (0.15×) are several-fold **faster than MRI**; `strftime` is at parity (1.05×). The TruffleRuby `Date` columns are cold-JIT outliers on these very short loops, not steady-state figures.

!!! note "Reproduce"
    The harness is committed under
    [`benchmarks/`](https://github.com/go-ruby-date/docs/tree/main/benchmarks):
    a self-contained Go driver (`go/`, pins the published library via
    `go.mod`), the equivalent `ruby/date.rb` workload, and `run.sh`. Run
