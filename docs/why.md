...
`go-ruby-date/date` reimplements Ruby's `Date` and `DateTime` in **pure Go, with cgo
...
- any Go program can import `github.com/go-ruby-date/date` directly, with no Ruby runtime;
